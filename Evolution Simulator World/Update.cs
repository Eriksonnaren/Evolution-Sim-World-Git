    ...
    public class UpdateWorld
    {
        ...
        public void MainUpdate()
        {
            for (int s = 0; s < Speed; s++)
            {
                ...
                if (Form1.NewTrees.Count < MinTrees)
                {
                    float angle = Form1.Rand.Next(0, 360);
                    float Dist = Form1.Sqrt((float)Form1.Rand.NextDouble()) * (Form1.ArenaRadius - 200);
                    float X = Form1.Cos(angle) * Dist;
                    float Y = Form1.Sin(angle) * Dist;
                    Form1.NewTrees.Add(new NewTree(new Vector(X, Y)));
                }
                ...
                for (int i = Form1.NewSeeds.Count - 1; i >= 0; i--)
                {
                    Form1.NewSeeds[i].Update();
                }
                ...
                for (int i = Form1.NewTrees.Count - 1; i >= 0; i--)
                {
                    if (Form1.NewTrees[i].Dead)
                        Form1.NewTrees.RemoveAt(i);
                }
                AverageCreatures += Form1.Creatures.Count;
                AverageTrees += Form1.NewTrees.Count;
                GraphTimer++;
